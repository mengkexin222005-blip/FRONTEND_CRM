...
const fillMinimum = (records, defaults) => {
  ...
  return [
    ...validRecords,
    ...availableDefaults.slice(0, MIN_ITEMS - validRecords.length),
  ];
};
...
export function useDashboard() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      const activeTasks = removeCompletedTasks(data?.tasksList);

      setStats({
        tasks: fillMinimum(activeTasks, defaultTasks),
        meetings: fillMinimum(data?.meetings, defaultMeetings),
      });
    } catch (err) {
